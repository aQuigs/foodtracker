...
- **No parallel fields for a single concept.** If you'd add a related field by copy-pasting and changing one word (`xPer100g`, `yPer100g`, …; `totals.x`, `totals.y`, …), use one field that holds the set instead (`per100g: Record<Nutrient, number>`, `totals: Record<Nutrient, number>`). Define the set as a single `const` registry of structs (`NUTRIENT_DEFS = [{ key, label, unit, … }] as const`); derive the union from it (`type Nutrient = typeof REG[number]['key']`) so the type cannot drift. Calc, validation, seed building, and UI rendering iterate over the registry. Adding a new member should be **one append to the registry**, plus a value on each seed — never edits across every layer.
