...
- **`npm run screenshots` captures the two main pages** (log + foods) at desktop (1280px), mid (700px), and narrow (480px) viewports into `screenshots/`. Run it after any UI change and analyze each PNG for: text wrapping, overflow, mis-aligned controls, missing labels, layout collapses, hover/active state collisions. Cheap to run, catches a lot.
- **Never tell the user a visual fix is done without re-running `npm run screenshots` AND reading the resulting PNGs.** "Fits on one line" is not "looks good"; "tests pass" is not "looks good"; measurements that say widths match is not "looks good." The only valid evidence is a fresh image of the affected viewports. If the user pushes back twice on the same visual issue, the iteration's problem is almost certainly *I didn't actually look* — re-screenshot before any further reply.
