...
async function loadClassifications(path: string): Promise<FoodClassification[]> {
  const parsed: unknown = JSON.parse(await readFile(path, 'utf8'));
  ...
  const bad = parsed.findIndex((e) => !isClassification(e));
  if (bad !== -1) {
    throw new Error(`${path}: entry ${bad} is not a valid classification: ${JSON.stringify(parsed[bad])}`);
  }

  return parsed as FoodClassification[];
}
...
async function main(): Promise<void> {
  ...
  if (mode === 'curated') {
    const curated = await loadCurated(listPath);
    ...
    const classifications = await loadClassifications(listPath);
  ...
  const outDir = join(PUBLIC_DATA_ROOT, `${sourceName}-v${version}`);
  ...
  process.stderr.write(`\nNext: commit public/data/${sourceName}-v${version}/* and push. GH Pages redeploys.\n`);
}
