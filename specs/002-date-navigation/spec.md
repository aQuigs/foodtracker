...
```
┌──────────────────────────────────────────┐
│ Food Tracker      [‹] [2026-05-23] [›]   │
│                          [Today]         │  shown only when ≠ today
├──────────────────────────────────────────┤
│ [ Search foods... ]                      │
│ [ Banana ]                               │
│ Grams: [ 100 ]            [ Log it ]     │
...
│ • Banana       120g    107 cal  [×]      │
├──────────────────────────────────────────┤
│ Total: 107 cal   Protein 1g   Carbs 27g   Fat 0g │
└──────────────────────────────────────────┘
```
...
9. Logging keeps a stable `loggedAt` timestamp distinct from the date field, so future M3/history features can sort by either
...
- One render path (state change → save → re-render) still holds. Date navigation changes are pure UI state changes that trigger paint().
