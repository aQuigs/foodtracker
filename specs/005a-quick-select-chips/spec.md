...
- Chips reflect `vm.logUnit` (the currently selected log unit), not the food's `servingUnit`.
...
Chips only appear after a food is selected. The row is hidden when no food is selected.
...
5. `chip-row` is hidden when `selectedFoodId` is `null`.
