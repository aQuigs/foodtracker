...
6. Catalog tab: hits whose id or name a live user food already has are hidden (counted per tier as `alreadyAdded`), so Add never offers an action the unique-name rule would refuse; the reducer enforces that rule for every write path, so the user can never be locked out of editing their own food. Add copies into `state.foods` and drops the row at once; re-adding a soft-deleted import revives it; tab hidden when no catalog is wired. A query that folds to nothing (only punctuation) shows the idle hint.
