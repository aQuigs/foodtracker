...
- Picking a recipe opens a card under its row headed `Each serving`: one line per item with an editable amount, the item's unit and live calories, all for one serving, then a Total line. A count that formats as 1 gives the plain one-serving Total, `Total 488 cal · P 40g · C 2g · F 36g`; otherwise the Total is the whole batch with one serving in brackets, `Total for 2 servings: 976 cal · P 80g · C 4g · F 72g (488 cal each)`. The count is formatted the way the logged group header formats it, and the batch is summed from the same scaled portions Log it writes and rounded once, so the card promises the calories the group header will show. The card is open exactly while the recipe is selected; clicking the selected row again, a search its row no longer matches, picking a food, or leaving the tab deselects it and brings Amount, Unit and the chips back.
...
Omelette  ⟨Recipe⟩                          ← picker row with tag
  ...
  Total for 2 servings: 976 cal · P 80g · C 4g · F 72g (488 cal each)
...
10. Card: changing Egg to 2 and Servings to 2 leaves the rows at one-serving calories and makes the Total read `Total for 2 servings: 592 cal · … (296 cal each)`; back at Servings 1 it reads `Total 296 cal`; a blank or 0 amount shows `—` and is skipped on log.
