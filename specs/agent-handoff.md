...
- `src/domain/foodLocks.ts` — `unitLock(state, current, next)`: refuses an edit that would strand a unit an entry or a live recipe portion already uses — an axis change, or removing pieces while something logs by count — shared by the reducer and the food form intent
