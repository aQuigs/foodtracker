...
- `src/domain/units.ts` — `DISPLAY_UNITS` (so far just `'fl oz'`) classifies units offered only for logging and recipe amounts, never as `Food.servingUnit` or a stored row's own `unit`: an `Entry`/`Portion` stays in its real, storable `unit` and carries an additive `shown` (`{ amount, unit }`) for how to display and re-edit it, read via `shownFor`. A pieces-based count works the same way — logging "4 count" of a food with pieces stores the physical grams `shown` was typed as, so a later edit to `pieces` can never rescale a past entry; only `resolvePickerAmount(amount, unit, food)` turns new input into a stored row, and only `scalePortion` (`src/ui/recipeIntents.ts`) may rescale one that already exists, from its own amount/shown ratio. `migrateCountEntries` (`src/domain/migrateCount.ts`) converts any row that still reads `'count'` against a food's current pieces — run inside `parseStateReport` so load and import both hold the invariant.
