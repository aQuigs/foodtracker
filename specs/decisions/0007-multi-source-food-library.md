...
- **Bundle the catalog in the JS payload** — even at ~380 KB gz it re-ships with every deploy and parses before any UI renders, and Branded-scale growth would make it unviable. Rejected. (The same-origin static asset under `public/data/` is *not* this: it's a separately-fetched file, not bundled into JS, and is gated by the IndexedDB cache check so existing users skip the network entirely.)
...
- **First-launch UX** has a one-time ~380 KB download with a progress banner. Subsequent launches are instant.
