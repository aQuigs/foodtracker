...
4. **The chart is inline SVG drawn by the app in pixels at the measured width.** Bars, one polyline, tick text. The chart factory owns a ResizeObserver and redraws itself from its last props. No chart library.
5. **Trends get their own tab**, with a metric toggle and a range toggle that share the unit picker's button-group factory.
...
- **viewBox scaling instead of measured width** — no observer, but non-uniform scaling distorts rounded ends and gaps and scales the tick text with the width. Rejected; the app column is 32 rem wide, so the observer rarely fires.
- **Macros stacked in calories** — the stack height would match the calorie chart and the donut's split, but goals, labels, the food form and the readout are all in grams. Rejected; grams are stacked and the readout shows both.
...
- There is no tooltip; the readout row is the one place values are read, which also keeps the card's geometry fixed.
