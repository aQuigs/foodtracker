...
3. **Navigations are network-first**, with a four-second timeout, falling back to the cached `index.html`. An online launch always runs the live deploy; a preview shows the push it was opened for. There is no "update available" prompt and no automatic reload.
...
6. **One cache per build, scoped by path:** `shell:<scope path>:<hash>`. Activation drops the same scope's other builds only, so the site and each preview, sharing one origin, never evict each other's shell. Install is atomic (`addAll`), then `skipWaiting` and `clients.claim`.
...
- On a connection that is up but stalled, the cached shell appears after four seconds.
