...
1. **A hand-written service worker, no workbox.** `src/sw/sw.ts`, the pure rules in `src/sw/routing.ts` and the fetch strategies in `src/sw/strategies.ts`, under two hundred lines, type-checked under the WebWorker lib in its own tsconfig. A Vite plugin (`scripts/serviceWorkerPlugin.ts`) runs a second, single-file build after the app's and emits `dist/sw.js` with the shell list and a content hash defined as `__SHELL__`.
...
- On a connection that is up but stalled, before or during the response, the cached shell appears after four seconds; on a server error, as soon as the error response has arrived. With no shell cached, the error page itself is served.
...
- A 404 at the site's own URL shows the host's page, not the cached shell: the rule that lets a closed preview read as gone applies to the site too.
