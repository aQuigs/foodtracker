...
1. **A brand is the unit of partition.** Each brand is a source `brand:<id>` that the user turns on individually. `brands/index.json` lists every brand as `[id, label, row count, file]`. The rows sit in 27 letter files (`a`…`z`, `0-9`), one for each first character of a brand id. Each letter file maps brand id to `{ label, rows }`. `brandFileKey(id)` in `src/domain/dataFiles.ts` names the file. The build and the app share that function, so hydrating a brand needs no list lookup: it fetches the letter file and keeps only its brand's entry. Each dump row lands in exactly one brand, so no two sources hold the same row. A brand ships rows only when it has at least two items or is a house brand. Every other brand is listed with its count and a `null` file. A brand search still finds it and shows it disabled, with the note "not included (1 item)".
2. **A row carries its brand.** `SourcedFood.brand` and `Food.brand` hold the label the row was built under. The tag, the brand half of the search text and food identity all read that field. Nothing infers a brand from a source name.
...
- **Commit the built data**, which is what this record first decided. Every file was reviewable in the PR and no build step was needed, but each rebuild added ~30 MB to git. Replaced by the CI build from pinned releases. A PR preview still shows its own data.
...
- Turning on one brand downloads its whole letter file, which is 3 KB to 960 KB gzipped (`s.json` is the largest). A store whose house brands share a letter requests that file once per brand.
- The brand list (~1.3 MB, 352 KB gzipped) is used only by the picker and for labels. It is fetched the first time a session needs it, which is when the picker opens or when a brand that is on hydrates. The manifest and the brand list are copied into IndexedDB beside the catalog, so an offline boot still searches what is cached and still shows brand names. If neither the network nor a copy is available, nothing is hydrated and the catalog shows its failure state.
- The IndexedDB catalog is a cache on an origin that PR previews share, so a database a newer build left at a higher schema is deleted and rebuilt.
