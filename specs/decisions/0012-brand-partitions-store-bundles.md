...
1. **A brand is the unit of partition.** One dataset, `brands`, holds every brand with a brand name in the dump: an index (`id`, label, row count, shard) and ~1,000 shards of ~100 KB. Each brand is a source `brand:<id>` that the user turns on individually; hydration fetches its shard and keeps only its rows. A dump row lands in exactly one brand, so no two sources can hold the same row.
...
5. **The whole dataset is committed**, as before: ~97 MB of JSON across ~1,000 files, which git stores as ~20 MB per release.
...
- The index (~1 MB) is one extra download per dataset version, on first picker open or on a boot with a brand on; it is kept in IndexedDB beside the catalog so offline boots still resolve labels.
