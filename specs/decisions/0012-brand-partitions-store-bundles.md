...
- Turning on one brand downloads its whole letter file, which is 3 KB to 960 KB gzipped (`s.json` is the largest). A letter file is fetched once per build however many of its brands download together, so a store whose house brands share a letter requests it once.
- The brand list (~1.3 MB, 353 KB gzipped) is read only by the picker, the first time it opens in a session. A brand that is on hydrates without it: its result fold reads the label its rows carry, and its download banner its id read as words. The manifest and the brand list are copied into IndexedDB beside the catalog. A brand-list copy at the manifest's version is used without a fetch, and an offline boot still searches what is cached. If neither the network nor a copy is available, nothing is hydrated and the catalog shows its failure state.
