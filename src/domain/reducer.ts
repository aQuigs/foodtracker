...
export function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'AddFood': {
      if (!isValidFood(action.food)) return state;
      if (state.foods.some((f) => f.id === action.food.id)) return state;
    ...
    case 'EditFood': {
      ...
      if (current.deletedAt !== null) return state;
      if (!isValidUpdates(action.updates)) return state;
