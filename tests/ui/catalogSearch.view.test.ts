...
describe('view — Catalog tab', () => {
  ...
  it('shows a distinct "no matches" message (not the idle hint) when catalogResults is an empty array', () => {
    render(container, { ...baseVm, view: 'catalog', catalogResults: [] }, noopHandlers);
  ...
  it('fires onImportFood with the sourced food id when Add is clicked', () => {
    ...
    render(container, { ...baseVm, view: 'catalog', catalogResults }, {
      ...noopHandlers,
...
describe('view — Catalog tab "More results" tier', () => {
  ...
  it('shows a collapsed More results toggle with the count', () => {
    render(container, { ...baseVm, view: 'catalog', catalogResults: tier1, catalogMoreResults: tier2 }, noopHandlers);
  ...
  it('does not claim "no matches" when the search itself failed', () => {
    render(container, {
      ...baseVm, view: 'catalog',
      catalogResults: [], catalogMoreResults: [], catalogError: 'boom',
    }, noopHandlers);
  ...
  it('caps the expanded deep tier at 200 rows and says how many are hidden', () => {
    ...
    render(container, {
      ...baseVm, view: 'catalog',
      catalogResults: tier1, catalogMoreResults: many, catalogMoreExpanded: true,
    }, noopHandlers);
  ...
  it('renders the catalog error directly above the results list, not below it', () => {
    render(container, {
      ...baseVm, view: 'catalog',
      catalogResults: [match(sourcedFood('usda:1', 'Apple', 52))], catalogError: 'boom',
    }, noopHandlers);
  ...
  it('fires onToggleCatalogMore when the toggle is clicked', () => {
    let fired = 0;
    render(container, { ...baseVm, view: 'catalog', catalogResults: tier1, catalogMoreResults: tier2 }, {
      ...noopHandlers,
  ...
  it('shows no toggle when there are no tier-2 matches', () => {
    render(container, { ...baseVm, view: 'catalog', catalogResults: tier1, catalogMoreResults: [] }, noopHandlers);
  ...
  it('lists tier-2 rows directly, with no toggle, when nothing curated matched', () => {
    render(container, { ...baseVm, view: 'catalog', catalogResults: [], catalogMoreResults: tier2 }, noopHandlers);
    expect(container.querySelector('[data-testid="catalog-more-toggle"]')).to.equal(null);
  ...
  it('shows the empty message only when both tiers are empty', () => {
    render(container, { ...baseVm, view: 'catalog', catalogResults: [], catalogMoreResults: [] }, noopHandlers);
    expect(container.querySelector('[data-testid="catalog-empty"]')).to.exist;
  });
