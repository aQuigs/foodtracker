...
function stateWithEntry(entry: Entry, foods = seedTestFoods()): State {
  return withMealsFromEntries({
    version: 2, enabledSources: defaultEnabledSources(), foods, meals: [], entries: [entry], recipes: [], recipeLogs: [],
...
describe('entry detail card rendering', () => {
  ...
  it('omits macro shares when no macro contributes calories', () => {
    ...
    const state: State = { version: 2, enabledSources: defaultEnabledSources(), meals: [], foods: zeroFood, entries: [bananaEntry], recipes: [], recipeLogs: [], settings: defaultSettings() };
    render(container, { ...baseVm, state: withMealsFromEntries(state), expandedDetail: { kind: 'entry', id: 'e1' } }, noopHandlers);
  ...
  it('still shares the macros of a food whose calorie line is 0', () => {
    ...
    const state: State = { version: 2, enabledSources: defaultEnabledSources(), meals: [], foods: carbsOnlyFood, entries: [bananaEntry], recipes: [], recipeLogs: [], settings: defaultSettings() };
    render(container, { ...baseVm, state: withMealsFromEntries(state), expandedDetail: { kind: 'entry', id: 'e1' } }, noopHandlers);
  ...
  it('only one detail card is mounted when expandedEntryId points to a single id', () => {
    const state: State = {
      ...
      entries: [bananaEntry, oatsEntry], recipes: [], recipeLogs: [], settings: defaultSettings(),
    };
  ...
  it('does not expand a row whose entry has invalid units', () => {
    ...
    const state: State = { version: 2, enabledSources: defaultEnabledSources(), meals: [], foods, entries: [bananaEntry], recipes: [], recipeLogs: [], settings: defaultSettings() };
    render(container, { ...baseVm, state: withMealsFromEntries(state), expandedDetail: { kind: 'entry', id: 'e1' } }, noopHandlers);
    expect(entryDetail(container)).to.equal(null);
