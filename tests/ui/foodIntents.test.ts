...
describe('parseFoodIntent — add', () => {
  ...
  it('rejects partial-blank chips (some filled, some empty)', () => {
    const r = parseFoodIntent(addInput({ name: 'Tofu', chipsRaw: ['80', '', '240', ''] }), existing, fixedClock());
    expect(r.kind).to.equal('error');
  });

  it('rejects chips with zero or negative values', () => {
    for (const chipsRaw of [['0', '160', '240', '320'], ['80', '-1', '240', '320'], ['80', '160', '240', '0']] as Array<[string, string, string, string]>) {
      const r = parseFoodIntent(addInput({ name: `Bad-${chipsRaw.join('-')}`, chipsRaw }), existing, fixedClock());
      expect(r.kind, `chipsRaw=${JSON.stringify(chipsRaw)}`).to.equal('error');
    }
  });

  it('rejects chips with non-numeric values', () => {
    const r = parseFoodIntent(addInput({ name: 'Soy', chipsRaw: ['80', 'abc', '240', '320'] }), existing, fixedClock());
    expect(r.kind).to.equal('error');
  });
});
