...
describe('parseFoodIntent — add', () => {
  ...
  it('accepts an explicit 0 in every nutrition field', () => {
    const r = parseFoodIntent({ mode: 'add', name: 'Water', calories: '0', protein: '0', carbs: '0', fat: '0', servingSize: '100', servingUnit: 'g' }, existing, [], fixedClock());
    expect(r.kind).to.equal('action');
  ...
  it('rejects a blank nutrition field instead of reading it as 0', () => {
    const r = parseFoodIntent({ mode: 'add', name: 'Banana', calories: '', protein: '', carbs: '', fat: '', servingSize: '100', servingUnit: 'g' }, [], [], fixedClock());
    expect(r.kind).to.equal('error');
    if (r.kind !== 'error') {
      throw new Error();
    }
